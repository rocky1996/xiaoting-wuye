...
for(int i = 0; i < chkbx.length; i++){
	if (lodgecomplaint.delete(Integer.parseInt(chkbx[i]))){
			mesg = "删除成功!";
...
keyword1 = lodgecomplaint.to_String(keyword1);

String keyword2 = request.getParameter("keyword2");
if (keyword2==null){ keyword2="";}else{mesg=mesg+keyword2;}
keyword2 = lodgecomplaint.to_String(keyword2);
...
<body bgcolor="#43CD80">
<center><form name="a1" method="post" action="lodgecomplaint.jsp?page=1">
...
<td width=10% align=left><font size=2 color=green><b>::投诉信息::</b></font></td>
<td width=70% align=center><font size=2 color=green>楼盘：<select name="keyword1" style="width:100">
...
&nbsp;投诉人：<INPUT type="text" NAME="keyword2" size=15 maxlength=40 value="<%= keyword2%>">&nbsp;<a href="#" style="color:green;" onclick="javascript:a1.submit();">查询</a>
</font></td>
...
</table></form><form name="a2" method="post" action="lodgecomplaint.jsp">
...
<% if (lodgecomplaint.lodgecomplaint_search(request)) {
	for (int i=0;i<lodgecomplaint.getlodgecomplaintlist().size();i++){
	     lodgecomplaint bk = (lodgecomplaint) lodgecomplaint.getlodgecomplaintlist().elementAt(i);
   if(lodgecomplaint.getrecordCount()<1){
	   %>&nbsp;<%
   }else{
%>
...
<td width=10% align="left" style="border-left:none; border-right:none; border-top:none; border-bottom:1 solid white">
<font size=2 color=green><%= bk.getlodgeusername() %></font></td>
<td width=30% align="left" style="border-left:none; border-right:none; border-top:none; border-bottom:1 solid white">
<font size=2 color=green><%= trun.truncate(bk.getreason(),25,"......") %></font></td>
<td width=12% align="left" style="border-left:none; border-right:none; border-top:none; border-bottom:1 solid white">
<%
if(acl.equals("管理员") ) {%>
<font size=2 color=green><a href="#" onclick="openScript('lodgecomplaint_modify.jsp?id=<%=bk.getId() %>','sop',660,405);">编辑
</a></font>&nbsp;
<font size=2 color=green><a href="#" onclick="openScript('lodgecomplaint_detail.jsp?id=<%=bk.getId() %>','sop',660,405);">查看</a></font>
<%}else{%>
<font size=2 color=green><a href="#" onclick="openScript('lodgecomplaint_detail.jsp?id=<%=bk.getId() %>','sop',660,405);">查看</a></font>
<%}%></td>
</tr>
...
<table align="center" width="95%" border="0" cellspacing="0" cellpadding="0"  bgcolor="#E4EDFB">
<tr bgcolor="#DEF3CE">
 <td width="50%" align="left" ><font size=2 color=green>&nbsp提示:共<%= lodgecomplaint.getrecordCount()%>条记录</font></td>
<td width="50%" align="right"><font size=2 color=green>共<%= lodgecomplaint.getPageCount() %>页&nbsp;当前页第<%= lodgecomplaint.getPage() %>页　
<a href="lodgecomplaint.jsp?keyword1=<%=keyword1%>&keyword2=<%=keyword2%>">首页</a>&nbsp;
<% if (lodgecomplaint.getPage()>1) {%>
<a href="lodgecomplaint.jsp?keyword1=<%=keyword1%>&keyword2=<%=keyword2%>&page=<%= lodgecomplaint.getPage()-1 %>">上一页</a>&nbsp;<% }else{ %>上一页&nbsp;<%}%>
<% if (lodgecomplaint.getPage()<lodgecomplaint.getPageCount()-1) {%>
<a href="lodgecomplaint.jsp?keyword1=<%=keyword1%>&keyword2=<%=keyword2%>&page=<%= lodgecomplaint.getPage()+1 %>">下一页</a>&nbsp;<% }else{ %>下一页<%}%>
<a href="lodgecomplaint.jsp?keyword1=<%=keyword1%>&keyword2=<%=keyword2%>&page=<%= lodgecomplaint.getPageCount() %>">未页</a>&nbsp;</font></td>
</tr>
</table>
<%}%>
</center>
